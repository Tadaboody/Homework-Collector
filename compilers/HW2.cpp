...
class Variable
{
  public:
	...
	Variable() {}
	Variable(address open_address, string type, int size, int offset) : var_address(open_address), offset(offset), type(type)
	{
...
class Pointer : public Variable
{
  public:
	...
	Pointer(address open_address, string type, int size, int offset, string member_type)
		: Variable(open_address, type, size, offset), member_type(member_type)
	{
	}
};
...
class Function
{ //the function you declare, not the one you pass as a var
  private:
...
void execute_code(AST *head, SymbolTable &table, int loop_num, ControlScope scope)
{
	...
	if (data == "break")
	{
		...
		case While:
			...
			break;
		case Switch:
			cout << "ujp switch_end_" << loop_num << endl;
			break;
		case None:
